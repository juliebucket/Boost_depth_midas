...
fid=fopen(fullfile(evaluation_matfile_save_dir,sprintf('evaluation_%s_clip.txt',evaluation_name)),'w');
...
for img=1:numel(imglist)
   ...
   % estimate_disp = cat(3,estimate_disp1,estimate_disp1,estimate_disp1);
    ...
    Q = quantile(estimate_depth(:),0.85);
    estimate_depth(estimate_depth>Q) = Q;
...
fprintf(fid,'ORD: %0.4f \n',mean(ord_error(:),'omitnan'))
fprintf(fid,'D3R: %0.4f \n',mean(d3r_error(:),'omitnan'))
fprintf(fid,'RMSE: %0.4f \n',mean(rmse_error(:),'omitnan'))
fprintf(fid,'THRESH 1.25: %0.4f \n',mean(thresh_1_25_error(:),'omitnan'))
fprintf(fid,'THRESH 1.25^2: %0.4f \n',mean(thresh_1_25_2_error(:),'omitnan'))
fprintf(fid,'THRESH 1.25^3: %0.4f \n',mean(thresh_1_25_3_error(:),'omitnan'))
fprintf(fid,'LOG10: %0.4f \n',mean(log_10(:),'omitnan'))
fprintf(fid,'ABS REL: %0.4f \n',mean(abs_rel(:),'omitnan'))
fprintf(fid,'SQ REL: %0.4f \n',mean(sq_rel(:),'omitnan'))
fclose(fid)